...
public class TreeBinary implements TBPrint {
    ...
    public void rotar(int value) {
        ...
        if (grandChild != null) { // Asumiendo una rotación derecha simple
            child.setRight(grandChild.getLeft()); // Mover el hijo izquierdo del nieto a ser el hijo derecho del hijo
            grandChild.setLeft(child); // Subir el nieto a la posición del hijo
            node.setLeft(grandChild); // Subir el nieto a conectar directamente con el nodo
        }

    }

    public Node getNextInOrden(Integer value) {
        Node node = findNode(root, value);
        if (node == null || !node.hasTwoSon()) {
            return null;
        }
        Node next = findMin(node.getRight());
        if (next != null) {
            System.out.println("Siguiente inOrden de " + value + " es " + next.getValue());
        }
        return next;
    }

    //Parte de la solucion nextInOrden
    ...
    //parte de la solucion de nextInOrden
...
//        System.out.println(tb.areSiblings(tb.root, 30, 28));
        ...
        System.out.println("Antes de usar la función rotar y getNextInOrden");

        tb.rotar(20);
        TBPrintUtil.print(tb);
        Node nextInOrder = tb.getNextInOrden(90);

        System.out.println(nextInOrder);



        System.out.println("Después de usar los métodos");
        TBPrintUtil.print(tb);
